...
\item[Layering on Observation] Monitoring Events
\item[Next Steps] Intervention Outcomes, Competition, Adaptation
...
TODO: INSERT PLOT AS BACKGROUND
\begin{itemize}
\item<2-> individuals convicted, connected by association, from public record prosecutorial data
\item<3-> how might this data be flawed?
\item<4-> if we take this as the model, even after adding roles, what do we know is unrepresented?
\item<5-> given those issues: does simulation on this network -- which includes deriving network statistics and predictions from them -- make sense?
\end{itemize}
}
...
\easyframe{PARAMETER ESTIMATES\cite{gong2012profitable}\cite{mcketin2005estimating}\cite{ritter2012evaluating}}{
